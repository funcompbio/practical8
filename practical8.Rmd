...
`read.csv()`. This data set contains information on the administered
...
Lists allow grouping values through their elements. Let's say we want
to group the number of administered doses in the previous data, by the vaccine
manufacturer. We can do that using the function `split()` to which we should 
give a first argument of the values we want to group and a second argument with 
the grouping factor.
...
to add back quote to delimit the name. We could also use double square brackets (`[[]]`)
...
We can use the function `sort()` to order area in hectares (Ha) from higher to 
lower:
...
`order()` function. The output of this function is vector of the positions each 
element in our vector would have if it was ordered. As it is a vector of 
positions, we can use it to arrange the rows of our data frame in that specific 
order:
...
**Exercise:** Order the data frame `pop_sel` according to the population of each municipality. Which are the most and less populated municipalities?

## Adding new columns

In some cases we might be interested in performing some operations using specific
columns of the data frame. For example, if we want to calculate the population 
density per km^2 using this, we can do as follows:
